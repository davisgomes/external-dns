...
func (cfg *Config) ParseFlags(args []string) error {
	...
	app.Flag("oci-auth-oke-workload-identity", "When using the OCI provider, specify whether OCI IAM workload identity authentication should be used (instead of key-based auth via the OCI config file).").Default(strconv.FormatBool(defaultConfig.OCIWorkloadIdentity)).BoolVar(&cfg.OCIWorkloadIdentity)
